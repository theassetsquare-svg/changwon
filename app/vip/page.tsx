 ...
 *
 *  이 주소(/vip)는 **이미 네이버에 색인돼 있다.** 그래서 주소는 그대로 두고
 *  안에 든 내용만 광고주 페이지로 바꿨다. 색인된 주소를 버리지 않기 위해서다.
 *
 *  ★ canonical·og:url 은 반드시 **이 주소(/vip)** 여야 한다.
 ...
 */
const VENUE = AD_BY_SLUG["daejeon-seven-night"];
const 이주소 = "/vip";
...
export const metadata: Metadata = adVariantMetadata(VENUE, 이주소, {
  description: 변형쪽들["/vip"].description,
});
export const viewport: Viewport = adViewport;

export default function Page() {
  return <AdNightPage venue={VENUE} 변형={변형쪽들["/vip"]} path={이주소} />;
}
