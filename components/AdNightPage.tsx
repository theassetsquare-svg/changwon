...
function 읽기전정리(v: AdVenue, 씨: string) {
  ...
  if (주소) {
    줄.push(주소틀[자리].replace("{A}", 주소) + (역 ? 역틀[(자리 + 2) % 6].replace("{S}", 역) : ""));
  }
  if (층) 줄.push(층틀[(자리 + 1) % 6].replace("{F}", 층));
  if (시간) 줄.push(시간틀[(자리 + 4) % 6].replace("{H}", 시간));
  줄.push(연령틀[(자리 + 3) % 6].replace("{G}", 연령 || "성인 · 신분증 확인"));
  줄.push(문의틀[(자리 + 5) % 6]);
  줄.push(`처음 가시는 분이라면 도착 시각과 일행 수를 먼저 정해 두시면 그날 움직임이 단순해집니다. ` +
    `자리를 고르는 기준은 사람마다 다르지만, 대화를 이어 가실 생각이면 소리가 덜 닿는 쪽을, ` +
    `분위기를 보고 싶으시면 홀이 한눈에 들어오는 쪽을 말씀하시면 됩니다.`);
  줄.push(`돌아오는 길도 미리 정해 두시길 권합니다. 늦은 시각에는 교통편이 달라지므로 ` +
    `출발 전에 한 번 확인해 두면 마무리가 편합니다. 차를 가져가실 생각이면 세울 곳을 함께 물어보시는 편이 좋습니다.`);
